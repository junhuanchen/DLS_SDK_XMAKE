...
XQDB::XQDB()
{
    m_DBfile="/data/user.db";
    m_ReCordDBfile="data/record.db";
...
bool XQDB::OpenDB_()
{
    ...
    if (m_db)
        return true;
    std::string db_name = m_DBfile.c_str();
...
bool XQDB::GetItemByFingerId(int finger_id, std::list<user_data> &p_user_datas)
{
    p_user_datas.clear();
    int ret = -1;
    sqlite3_stmt *stmt;

    const char *sql_ = "select * from tb_user_data where finger_id = %d;";    

    char sql[1024];
    snprintf(sql, 1023, sql_, finger_id);
    ret = sqlite3_prepare(m_db, sql, strlen(sql), &stmt, NULL);
    if (SQLITE_OK != ret)
    {
        printf("sqlite3_prepare fail(GetItemByUserId)\n");
...
int XQDB::GetCardByUserId(int user_id,const char *card)
{
    int ret = -1;
    sqlite3_stmt *stmt;
    char szbuf[1024];
